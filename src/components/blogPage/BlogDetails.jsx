...
const BlogDetails = () => {
  ...
  useEffect(() => {
    const post = blogPostList.find((p) => p.id === parseInt(id)); // Find the post by id
    setSelectedPost(post); // Update the selectedPost state
  }, [id]); // Re-run the effect when `id` changes

  if (!selectedPost) {
    return <h1 className="text-center text-2xl">Blog Post Not Found</h1>;
  }
  ...
  return (
    <div className="flex flex-col md:flex-row max-w-6xl mx-auto p-6 gap-10" style={{ marginTop: "140px" }}>
      {/* Blog Post Content */}
      <div className="flex-1">
        <h1 className="text-4xl font-bold mb-4 text-gray-900">{selectedPost.title}</h1>
        <p className="text-gray-600 mb-6 text-sm">
          {selectedPost.date} • {selectedPost.category} • David Jones
        </p>
        <img
